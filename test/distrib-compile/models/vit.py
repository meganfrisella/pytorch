...
# torch._dynamo.disallow_in_graph(einops.rearrange)
# torch._dynamo.disallow_in_graph(einops.repeat)
# torch._dynamo.disallow_in_graph(einops.reduce)
# torch._dynamo.disallow_in_graph(torch.unique)
...
class AdaptiveTokenSampling(nn.Module):
    ...
    def forward(self, attn, value, mask):
        ...
        # print(pseudo_logits.shape, mask_without_cls.shape)
        pseudo_logits = pseudo_logits.masked_fill(~mask_without_cls, mask_value)

        # expand k times, k being the adaptive sampling number

        # print(pseudo_logits.shape, output_num_tokens)
        pseudo_logits = repeat(pseudo_logits, 'b n -> b k n', k = output_num_tokens)
        # pseudo_logits = pseudo_logits.expand([pseudo_logits.shape[0], output_num_tokens, pseudo_logits.shape[1]])
        ...
        # print(expanded_unique_sampled_token_ids.shape, heads)
        expanded_unique_sampled_token_ids = repeat(unique_sampled_token_ids, 'b n -> b h n', h = heads)
        # expanded_unique_sampled_token_ids = unique_sampled_token_ids.expand(unique_sampled_token_ids.shape[0], heads, unique_sampled_token_ids.shape[1])
...
class ViT(nn.Module):
    ...
    def forward(self, img, return_sampled_token_ids = False, dynamo_mb=0):
